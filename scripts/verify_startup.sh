...
VERIFICATION_PROJECT_NAME="label-forge-verify-${PROFILE}-$$"
VERIFICATION_ENV_FILE="$(mktemp "${TMPDIR:-/tmp}/label-forge-verify-${PROFILE}-XXXXXX.env")"
...
main() {
  if [[ ! -x "${REPO_ROOT}/manage_label_forge.sh" ]]; then
    echo "ERROR: Management script was not found or is not executable: ${REPO_ROOT}/manage_label_forge.sh" >&2
    return 1
  fi
  ...
  echo "[INFO] Running Label-Forge startup verification..."
